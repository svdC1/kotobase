...
jmdict_kanji_assoc = Table('jmdict_kanji_assoc', Base.metadata,
    Column('jmdict_id', Integer, ForeignKey('jmdict_entries.id')),
    Column('kanji_id', Integer, ForeignKey('jmdict_kanji.id'))
)

jmdict_kana_assoc = Table('jmdict_kana_assoc', Base.metadata,
    Column('jmdict_id', Integer, ForeignKey('jmdict_entries.id')),
    Column('kana_id', Integer, ForeignKey('jmdict_kana.id'))
)
...
class JMDictEntry(Base):
    ...
    kanji = relationship("JMDictKanji", secondary=jmdict_kanji_assoc, back_populates="entries")
    kana = relationship("JMDictKana", secondary=jmdict_kana_assoc, back_populates="entries")
    senses = relationship("JMDictSense", back_populates="entry", order_by="JMDictSense.order")

class JMDictKanji(Base):
    __tablename__ = 'jmdict_kanji'
    id = Column(Integer, primary_key=True)
    text = Column(String, unique=True)
    entries = relationship("JMDictEntry", secondary=jmdict_kanji_assoc, back_populates="kanji")

class JMDictKana(Base):
    __tablename__ = 'jmdict_kana'
    id = Column(Integer, primary_key=True)
    text = Column(String, unique=True)
    entries = relationship("JMDictEntry", secondary=jmdict_kana_assoc, back_populates="kana")

class JMDictSense(Base):
    __tablename__ = 'jmdict_senses'
    id = Column(Integer, primary_key=True)
    entry_id = Column(Integer, ForeignKey('jmdict_entries.id'))
