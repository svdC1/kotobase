...
def parse_kanjidic():
    ...
    for _, element in etree.iterparse(raw_path, tag='character'):
        character = {
            ...
            "stroke_count": [sc.text for sc in element.findall('misc/stroke_count')],
            "variants": [
            ...
            "dic_number": [
                {"type": dr.get('dr_type'), "m_vol": dr.get('m_vol'), "m_page": dr.get('m_page'), "value": dr.text}
                for dr in element.findall('dic_number/dic_ref')
            ],
            "query_code": [
                {"type": qc.get('qc_type'), "skip_misclass": qc.get('skip_misclass'), "value": qc.text}
                for qc in element.findall('query_code/q_code')
            ],
            "reading_meaning": {
                "readings": [
                    {"type": r.get('r_type'), "on_type": r.get('on_type'), "r_status": r.get('r_status'), "value": r.text}
                    for r in element.findall('reading_meaning/rmgroup/reading')
                ],
                "meanings": [
                    {"lang": m.get('m_lang', 'en'), "value": m.text}
    ...
    print(f"Writing {len(characters)} characters to {processed_path}...")
    with open(processed_path, 'w', encoding='utf-8') as f:
        json.dump(characters, f, ensure_ascii=False)

    print("Successfully processed Kanjidic2.")
